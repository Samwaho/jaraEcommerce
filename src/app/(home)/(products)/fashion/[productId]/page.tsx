...
import { getFashionById } from "@/lib/actions";
import Image from "next/image";
import React from "react";

const Page = async ({ params }: { params: { productId: string } }) => {
  const fashion = await getFashionById(params.productId);
  return (
    <div className="p-4 min-h-screen bg-main-bg pb-14">
      <div className=" -mt-4 bg-white shadow-md rounded-lg p-3 ">
        ...
        <Button className="h-6">{fashion?.condition}</Button>
      </div>
      <SellerDetails sellerId={fashion?.sellerId || ""} />
      <RateSeller />
      <RateSellerService sellerId={fashion?.sellerId || ""} />
      <Comments />
    </div>
  );
};
