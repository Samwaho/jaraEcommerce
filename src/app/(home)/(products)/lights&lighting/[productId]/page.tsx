...
const Page = async ({ params }: { params: { productId: string } }) => {
  const lights = await getLightsById(params.productId);
  return (
    <div className="p-4 min-h-screen bg-main-bg pb-14">
      <div className=" -mt-4 bg-white shadow-md rounded-lg p-3 ">
        ...
        <h2 className="mt-4 font-bold text-xl">{`Ksh ${lights?.price}`}</h2>
        ...
        <Button className="h-6">{lights?.condition}</Button>
      </div>
      <SellerDetails sellerId={lights?.sellerId || ""} />
      <RateSeller />
      <RateSellerService sellerId={lights?.sellerId || ""} />
      <Comments />
    </div>
  );
};
