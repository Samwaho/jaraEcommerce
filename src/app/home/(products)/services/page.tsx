...
import { getServicesProducts } from "@/lib/actions";
import Image from "next/image";
import Link from "next/link";
import React from "react";

const Page = async () => {
  const products = await getServicesProducts();
  return (
      ...
      <div className="p-2">
        <MasonryGrid>
          {products?.map((product) => {
            return (
              <Link key={product.id} href={`/home/services/${product.id}`}>
                <div className="p-3 mt-2 flex flex-col gap-2 rounded-xl shadow-md bg-white">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center justify-between gap-6">
                      ...
                      <p className="text-sm truncate">{product.occupation}</p>
                    </div>
                    <div className="flex items-center justify-between gap-6">
                  ...
                  <Button className="w-16 h-4">NEW</Button>
                </div>
              </Link>
            );
          })}
